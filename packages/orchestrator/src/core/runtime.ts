...
async function cleanupOrphanedWorkers(): Promise<void> {
	...
	for (const [pid, group] of byPid.entries()) {
		...
		if (anyWithHost) {
			const hostAlive = group.some(
				(e) => typeof e.hostPid === "number" && isProcessAlive(e.hostPid),
			);
			if (hostAlive) {
			...
			// Entries without hostPid: only clean up if they're stale and clearly not ours.
			...
			if (
				instanceId &&
				group.some((e) => e.orchestratorInstanceId === instanceId)
			) {
				liveWorkerPids.add(pid);
				continue;
			}
...
async function cleanupOrphanedServeProcesses(
	liveWorkerPids: Set<number>,
): Promise<void> {
		...
		workers.map(async (w) => {
			try {
				...
				if (typeof w.pid === "number")
					await removeWorkerEntriesByPid(w.pid).catch(() => {});
			}
		}),
	);
